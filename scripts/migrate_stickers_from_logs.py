#!/usr/bin/env python3
"""
One-time migration: parse chat log files for [STICKER] entries and insert them
into the messages table so sticker counts work in /stats, /mystats, /report.

Inserts rows with:
  - negative message_id (below current minimum) to avoid conflicts with real IDs
  - raw_telegram_message = '{"sticker": {}}' so the existing queries match
  - text = NULL, is_command = false
  - user_id looked up from the users table by username

Run once: python scripts/migrate_stickers_from_logs.py
"""
...
# Matches lines like:
# 2026-02-19 11:38:33,998 +0200 - chat - INFO - Ctx:[-1002096701815][supergroup][Psychochauffeur Club][vo1dee] - [STICKER] 😅
LOG_RE = re.compile(
...
def parse_log_files():
    """Yield (timestamp_utc, chat_id, username) for every [STICKER] line in all logs."""
...
async def main():
    ...
    # Collect all sticker entries, deduplicate by (chat_id, timestamp, username)
    ...
    print(f"Found {len(entries)} sticker log lines → {len(deduped)} unique entries")

    # Check which (chat_id, timestamp, username) are already in the DB as stickers
    existing = await conn.fetch(
        "SELECT chat_id, timestamp, u.username "
        "FROM messages m JOIN users u ON m.user_id = u.user_id "
        "WHERE raw_telegram_message ? 'sticker'"
    )
    already = {(r["chat_id"], r["timestamp"].replace(tzinfo=pytz.UTC), r["username"].lower() if r["username"] else "") for r in existing}
    print(f"Already have {len(already)} sticker rows in DB")

    skipped_no_user = 0
    skipped_exists = 0
    inserted = 0
    raw_sticker = json.dumps({"sticker": {}})
    ...
    async with conn.transaction():
        for ts, chat_id, username in deduped:
            ...
            """, next_id, chat_id, user_id, ts, raw_sticker)
            next_id -= 1
            inserted += 1
