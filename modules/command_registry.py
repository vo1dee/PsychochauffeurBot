...
class CommandRegistry(ServiceInterface):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self.service_registry:
                try:
                    self._config_manager = self.service_registry.get_service(
                        "config_manager"
                    )
    ...
    def register_command(self, command_info: CommandInfo) -> "CommandRegistry":
        ...
        for alias in command_info.aliases:
            if alias in self._aliases:
                logger.warning(
                    f"Alias '{alias}' is already registered for command '{self._aliases[alias]}', overwriting"
                )
        ...
        logger.info(
            f"Registered command: /{command_name} (category: {command_info.category.value})"
        )
        if command_info.aliases:
            logger.info(
                f"  Aliases: {', '.join(f'/{alias}' for alias in command_info.aliases)}"
            )
    ...
    async def _register_telegram_handlers(self) -> None:
        ...
        logger.info(
            f"Registered {len(self._commands)} command handlers with Telegram application"
        )
    ...
    async def register_utility_commands(self) -> None:
        ...
        # Short command (download YouTube Shorts as video)
        self.register_command(
            CommandInfo(
                name="short",
                description="Download YouTube Shorts as video (reply to a message with Shorts link)",
                category=CommandCategory.UTILITY,
        ...
        self.register_command(
            CommandInfo(
                ...
                examples=[
                    "/mute @username 30",
                    "/mute 123456789 30",
                    "/mute 60 (when replying to a message)",
                ],
            )
        )
        ...
        self.register_command(
            CommandInfo(
                ...
                examples=[
                    "/unmute @username",
                    "/unmute 123456789",
                    "/unmute (when replying to a message)",
                ],
            )
        )
    ...
    async def register_speech_commands(self) -> None:
        ...
        try:
            if self.service_registry:
                speech_service = self.service_registry.get_service(
    ...
    def validate_command_registration(self) -> List[str]:
        ...
        for name, command_info in self._commands.items():
            if (
                not hasattr(command_info, "handler_func")
                or command_info.handler_func is None
            ):
    ...
    async def _load_service_configuration(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded service configuration: {len(self._service_config)} settings"
            )
    ...
    async def _handle_configuration_change(
        self, module_name: str, new_config: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            modified_keys = {
                key
                for key in self._service_config.keys() & old_config.keys()
