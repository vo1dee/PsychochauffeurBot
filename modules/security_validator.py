...
class InputSanitizer:
    ...
    # WARNING: Regex-based HTML filtering is inherently unsafe due to browser
    # HTML parsing quirks. Use sanitize_html() method for robust HTML sanitization.
    DANGEROUS_PATTERNS = {
        ...
        'script_injection': re.compile(
            ...
            r'on\w+\s*=|'
            r'<iframe|<object|<embed|<link|<meta|<style',
    ...
    @classmethod
    def sanitize_html(cls, text: str, allowed_tags: Optional[Set[str]] = None) -> str:
        """
        Sanitize HTML content using BeautifulSoup for robust parsing.
        ...
        try:
            ...
            for tag in soup.find_all():
                ...
                for attr in tag.attrs:
                    ...
                    if attr_lower.startswith('on'):
                        dangerous_attrs.append(attr)
                    # Remove javascript: and data: URLs
                    elif attr_lower in ['href', 'src', 'action', 'formaction']:
                        value = str(tag.attrs[attr]).lower()
                        if any(protocol in value for protocol in ['javascript:', 'vbscript:', 'data:']):
                            dangerous_attrs.append(attr)
            ...
            logger.error(f"Error in HTML sanitization: {e}")
            # Fallback to basic entity encoding
            for char, entity in cls.HTML_ENTITIES.items():
                text = text.replace(char, entity)
            return text
    ...
    def sanitize_text(cls, text: Any, strict: bool = False) -> str:
        ...
        if strict:
            # Use robust HTML sanitization instead of basic regex
            text = cls.sanitize_html(text, allowed_tags=set())  # No HTML tags allowed in strict mode
    ...
    def sanitize_filename(cls, filename: Any) -> str:
        ...
        if len(filename) > SecurityConstants.MAX_FILENAME_LENGTH:
            ...
            max_name_length = (SecurityConstants.MAX_FILENAME_LENGTH - len(ext) - 1
                              if ext else SecurityConstants.MAX_FILENAME_LENGTH)
    ...
    def detect_threats(cls, text: str) -> List[ThreatType]:
        ...
        # Additional robust HTML threat detection using BeautifulSoup
        if cls._detect_html_threats(text):
            threats.append(ThreatType.INJECTION_ATTEMPT)

        return threats

    @classmethod
    def _detect_html_threats(cls, text: str) -> bool:
        """
        Detect HTML-based threats using robust parsing.
        This complements the regex-based detection with safer parsing.
        """
        if not HAS_BEAUTIFULSOUP:
            return False
        ...
        try:
            ...
            for tag in soup.find_all():
                if tag.name and tag.name.lower() in dangerous_tags:
                    return True
                ...
                for attr, value in tag.attrs.items():
                    ...
                    if attr_lower in ['href', 'src', 'action', 'formaction']:
                        if any(protocol in value_lower for protocol in
            ...
            logger.debug(f"Error in HTML threat detection: {e}")
            return False
