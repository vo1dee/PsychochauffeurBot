...
class Stickers:
    ...
    ALIEXPRESS: str = (
        "CAACAgQAAxkBAAEuNplnAqatdmo-G7S_065k9AXXnqUn4QACwhQAAlKL8FNCof7bbA2jAjYE"
    )
    LOCATION: str = (
        "CAACAgQAAxkBAAE3R61oaOg4WXbVWO3aeHPbKFcvKR8JRAACMhsAAl0KGFDROvkaE3ex5jYE"
    )
...
class InstagramConfig:
    ...
    MAX_RETRIES = int(
        os.getenv("INSTAGRAM_MAX_RETRIES", "6")
    )  # Double the default for Instagram
    RETRY_DELAY_BASE = int(
        os.getenv("INSTAGRAM_RETRY_DELAY", "2")
    )  # Base delay in seconds
    RETRY_BACKOFF_MULTIPLIER = float(
        os.getenv("INSTAGRAM_BACKOFF_MULTIPLIER", "2.0")
    )  # Exponential backoff multiplier
    MAX_RETRY_DELAY = int(
        os.getenv("INSTAGRAM_MAX_RETRY_DELAY", "30")
    )  # Maximum delay cap
